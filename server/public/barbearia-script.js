...
function homePage() {
  ...
  return `
    ...
    <div class="hero">
      ...
      <a href="#agendar" class="btn btn-primary">📅 Agende seu horario</a>
    </div>
    ...
    <div class="container">
      ...
      <div class="section">
        <div class="section-title">💇 <span>Servicos</span> <span class="badge">${servicos.length}</span></div>
        ...
        `).join('')}
        ${servicos.length > 4 ? `<a href="#servicos" class="btn btn-secondary mt-sm">Ver todos os servicos</a>` : ''}
      </div>
...
function servicosPage() {
  ...
  return `
    ...
    <div class="container" style="padding-top:24px">
      <h2 style="font-size:22px;margin-bottom:20px">💇 Nossos Servicos</h2>
      ${categorias.map(cat => `
      ...
      `).join('') : ''}
      ${servicos.length === 0 ? '<p style="color:var(--text-muted);text-align:center;padding:40px 0">Nenhum servico cadastrado ainda</p>' : ''}
      <a href="#agendar" class="btn btn-primary mt-md">📅 Agendar horario</a>
    </div>
    ${footerHTML()}`;
}
...
async function agendarPage(step) {
  ...
  // Se recebeu servico via URL, pre-seleciona
...
function agendarStep1(servicos) {
  return `
    <h2 style="font-size:22px;margin-bottom:4px">Escolha o servico</h2>
    <p style="color:var(--text-muted);font-size:14px;margin-bottom:20px">Selecione o que voce precisa</p>
    ${servicos.map(s => `
...
function agendarStep2(servicos) {
  ...
  return `
    <div class="alert alert-info">💇 Servico escolhido: <strong>${s.nome}</strong> (${preco(s.preco)})</div>
    <h2 style="font-size:22px;margin-bottom:4px">Quase la!</h2>
    ...
    <div style="display:flex;gap:12px;margin-top:24px">
      ...
      <button class="btn btn-primary" style="flex:2 ${!selectedProf ? 'btn-disabled' : ''}"
              onclick="navigate('#agendar?step=3')">Escolher horario →</button>
    </div>
  `;
}
...
async function agendarStep3(profissionais) {
  ...
  // Gera slots de 30min das 8h as 19h
  const horarios = [];
  for (let h = 8; h <= 19; h++) {
  ...
  return `
    <h2 style="font-size:22px;margin-bottom:4px">Escolha o horario</h2>
...
function agendarStep6() {
  return `
    <div class="sucesso-page">
      ...
      <p>Voce recebera uma confirmacao no WhatsApp.<br>Se precisar alterar ou cancelar, e so consultar "Meus Agendamentos".</p>
      <a href="#meus-agendamentos" class="btn btn-primary" style="max-width:280px;margin:0 auto">📋 Ver meus agendamentos</a>
      <a href="#home" class="btn btn-secondary mt-sm" style="max-width:280px;margin:12px auto 0">🏠 Voltar ao inicio</a>
    </div>
  `;
}
  ...
  return `
    ...
    <div class="container" style="padding-top:24px">
      ...
      <div class="phone-search" id="phone-search">
        ...
        <p>Digite o mesmo numero que usou para agendar</p>
        <div class="form-group">
    ...
    if (!dados.agendamentos || dados.agendamentos.length === 0) {
      resultEl.innerHTML = `
        <div class="alert alert-info" style="text-align:center;padding:40px">
          ...
          <p style="color:var(--text-muted);margin-top:8px">Esse numero nao tem agendamentos ou voce ainda nao agendou conosco.</p>
        ...
        return `
          <div class="agendamento-card ${statusClass}">
            <div class="ag-header">
              ...
              <span class="ag-status ${a.status}">${a.status === 'agendado' ? '✅ Agendado' : a.status === 'cancelado' ? '❌ Cancelado' : '✔️ Concluido'}</span>
            </div>
            <div class="ag-detalhes">
              <div>📅 ${formatData(a.data_hora.split('T')[0])} as ${formatHora(a.data_hora)}</div>
