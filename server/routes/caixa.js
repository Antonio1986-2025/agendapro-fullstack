...
router.get('/', async (req, res) => {
  ...
  const caixa = await query(
    `SELECT * FROM caixa_registros WHERE barbearia_id = $1 AND data = ${dia === 'CURRENT_DATE' ? 'CURRENT_DATE' : `$$2`}`,
    dia === 'CURRENT_DATE' ? [req.barbeariaId] : [req.barbeariaId, dia]
  );
  ...
  res.json({
    ...
    saldo: (parseFloat(caixa.rows[0].valor_inicial) + entradas - saidas).toFixed(2),
  });
});
